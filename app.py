...
def normalize_colname(s: str) -> str:
    s = unicodedata.normalize("NFKD", s).encode("ascii", "ignore").decode("ascii")
    s = s.strip().lower()
@@ -34,27 +16,22 @@ def normalize_colname(s: str) -> str:
...
RUNS_EXPECTED: Dict[str, list[str]] = {
    ...
    "att_runs_pm": ["count pass attempts for runs per match"],
    "comp_ratio_runs": ["pass completion ratio to runs"],
@@ -66,10 +43,9 @@ def load_csv(file: io.BytesIO, semicolon: bool, comma_decimal: bool) -> pd.DataF
    "att_danger_runs_pm": ["count pass attempts for dangerous runs per match"],
    "completed_danger_runs_pm": ["count completed passes for dangerous runs per match"],
}
...
PRESS_EXPECTED: Dict[str, list[str]] = {
    ...
    "channel": ["channel"],
    "minutes_pm": ["minutes played per match"],
@@ -81,253 +57,156 @@ def load_csv(file: io.BytesIO, semicolon: bool, comma_decimal: bool) -> pd.DataF
    "retentions_press_pm": ["count ball retentions under pressure per match"],
    "forced_losses_press_pm": ["count forced losses under pressure per match"],
}
...
with TAB_RUNS:
    ...
    if runs_df_raw.empty:
        ...
        for canon, aliases in RUNS_EXPECTED.items():
            ...
            for alias in aliases + [canon]:
                ...
                if alias_norm in norm_cols:
                    found = norm_cols[alias_norm]
                    break
                if alias_norm in norm_cols: found = norm_cols[alias_norm]; break
                for k_norm, orig in norm_cols.items():
                    if alias_norm in k_norm:
                        found = orig
                        break
                if found:
                    break
                    if alias_norm in k_norm: found = orig; break
                if found: break
            auto_map[canon] = found
        ...
        cols = st.columns(3)
        items = list(RUNS_EXPECTED.keys())
        cols = st.columns(3); items = list(RUNS_EXPECTED.keys())
        for i, canon in enumerate(items):
            with cols[i % 3]:
                options = [None] + list(runs_df_raw.columns)
                mapping[canon] = st.selectbox(
        ...
        required = [
            ...
            "opp_danger_runs_pm", "att_danger_runs_pm", "completed_danger_runs_pm",
                ...
                mapping[canon] = st.selectbox(f"{canon}", options=options, index=(options.index(auto_map[canon]) if auto_map[canon] in options else 0), key=f"map_runs_{canon}")
        required = ["player","opp_runs_pm","att_runs_pm","comp_ratio_runs","threat_completed_pm","completed_runs_pm","completed_to_shot_pm","completed_to_goal_pm","opp_danger_runs_pm","att_danger_runs_pm","completed_danger_runs_pm"]
        missing = [c for c in required if mapping.get(c) is None]
        if missing:
            st.error("Missing required columns: " + ", ".join(missing))
        else:
            ...
            # Percentiles
            df["attempt_rate_runs"] = df["att_runs_pm"] / nz(df["opp_runs_pm"])
            df["attempt_rate_danger"] = df["att_danger_runs_pm"] / nz(df["opp_danger_runs_pm"])
            df["productivity_runs"] = df["completed_runs_pm"]
            df["conversion_to_shot"] = df["completed_to_shot_pm"] / nz(df["completed_runs_pm"])
            df["conversion_to_goal"] = df["completed_to_goal_pm"] / nz(df["completed_runs_pm"])
            df["exp_threat_per_attempt"] = df["threat_completed_pm"] / nz(df["att_runs_pm"])
            df["dangerous_completion"] = df["completed_danger_runs_pm"] / nz(df["att_danger_runs_pm"])
            df["dangerous_attempt_share"] = df["att_danger_runs_pm"] / nz(df["att_runs_pm"])
            pct = lambda s: s.rank(pct=True, na_option="keep")
            rank_cols = [
                "attempt_rate_runs","comp_ratio_runs","threat_completed_pm","productivity_runs",
                "conversion_to_shot","conversion_to_goal","exp_threat_per_attempt","dangerous_completion","dangerous_attempt_share"
            ...
            for col in rank_cols:
            for col in ["attempt_rate_runs","comp_ratio_runs","threat_completed_pm","productivity_runs","conversion_to_shot","conversion_to_goal","exp_threat_per_attempt","dangerous_completion","dangerous_attempt_share"]:
                df[f"p_{col}"] = pct(df[col])
            ...
            # ---- Filters
            df["creator_index"] = 0.30*df["p_threat_completed_pm"] + 0.20*df["p_productivity_runs"] + 0.20*df["p_comp_ratio_runs"] + 0.20*df["p_attempt_rate_runs"] + 0.10*df["p_conversion_to_shot"]
            df["creator_index_v2"] = 0.35*df["p_exp_threat_per_attempt"] + 0.25*df["p_threat_completed_pm"] + 0.15*df["p_comp_ratio_runs"] + 0.15*df["p_attempt_rate_runs"] + 0.10*df["p_dangerous_completion"]
            st.markdown("### Filters")
            ...
            if sel_channel:
                mask &= df["channel"].isin(sel_channel)
            if not pd.isna(df["minutes_pm"]).all(): mask &= (df["minutes_pm"].fillna(0) >= min_minutes)
            if sel_teams: mask &= df["team"].isin(sel_teams)
            if sel_third: mask &= df["third"].isin(sel_third)
            if sel_channel: mask &= df["channel"].isin(sel_channel)
            df_view = df.loc[mask].copy()
            ...
            cols_creator = ["player","team","third","channel","opp_runs_pm","att_runs_pm","attempt_rate_runs","comp_ratio_runs","threat_completed_pm","exp_threat_per_attempt","completed_runs_pm","completed_to_shot_pm","completed_to_goal_pm","dangerous_attempt_share","dangerous_completion","creator_index","creator_index_v2"]
            leader_creator = df_view.sort_values("creator_index_v2", ascending=False)[cols_creator].head(n_top)
            st.dataframe(leader_creator.round(3), use_container_width=True)
            st.download_button("Download CSV — Runs creators (v2)", leader_creator.to_csv(index=False).encode("utf-8"), file_name="runs_creators_v2.csv")
            ...
            c1,c2,c3 = st.columns([1,1,1])
            with c1: chart_w = st.number_input("Chart width (px)", 800, 4000, 1600, step=100, key="runs_w")
            with c2: chart_h = st.number_input("Chart height (px)", 400, 2400, 900, step=50, key="runs_h")
            with c3: point_size = st.number_input("Point size", 20, 300, 90, step=5, key="runs_pts")
            color_choice = st.selectbox("Color points by", ["team","third","channel","None"], index=0, key="runs_color")
            ...
            def _apply_color(enc):
                if color_choice != "None":
                    if color_choice == "team":
                        return enc.encode(color=alt.Color("team:N", scale=TEAM_COLOR_SCHEME, legend=alt.Legend(title="Team")))
                    if color_choice == "team": return enc.encode(color=alt.Color("team:N", scale=TEAM_COLOR_SCHEME, legend=alt.Legend(title="Team")))
                    return enc.encode(color=alt.Color(f"{color_choice}:N"))
                return enc
            ...
            base = alt.Chart(df_view.dropna(subset=["attempt_rate_runs","comp_ratio_runs"])).mark_circle(size=point_size).encode(
                x=alt.X("attempt_rate_runs:Q", title="Attempt rate to runs (attempts / opportunities)"),
                y=alt.Y("comp_ratio_runs:Q", title="Pass completion to runs"),
                tooltip=["player","team","attempt_rate_runs","comp_ratio_runs","threat_completed_pm","completed_runs_pm"],
            )
            ...
            chart2 = alt.Chart(df_view.dropna(subset=["exp_threat_per_attempt","attempt_rate_runs"])) \
                ...
                .properties(width=chart_w, height=chart_h, title="Risk–Reward: Attempts vs Value per Attempt")
            chart2 = alt.Chart(df_view.dropna(subset=["exp_threat_per_attempt","attempt_rate_runs"])).mark_circle(size=point_size).encode(
                x=alt.X("attempt_rate_runs:Q", title="Attempt rate to runs"),
                y=alt.Y("exp_threat_per_attempt:Q", title="Expected threat per attempt"),
                tooltip=["player","team","exp_threat_per_attempt","attempt_rate_runs","threat_completed_pm"],
                color=alt.Color("dangerous_attempt_share:Q", title="Dangerous share", scale=alt.Scale(scheme='blues'))
            ...
            chart3 = alt.Chart(df_view.dropna(subset=["dangerous_attempt_share","dangerous_completion"])).mark_circle(size=point_size).encode(
                x=alt.X("dangerous_attempt_share:Q", title="Dangerous attempt share"),
                y=alt.Y("dangerous_completion:Q", title="Dangerous pass completion"),
                tooltip=["player","team","dangerous_attempt_share","dangerous_completion"],
            )
            ...
            chart4 = alt.Chart(df_view.dropna(subset=["comp_ratio_runs"])) \
                ...
                .encode(x=alt.X('comp_ratio_runs:Q', title='Completion to runs'), y='density:Q') \
                .properties(width=chart_w, height=int(chart_h*0.6), title="Distribution — Pass completion to runs")
            chart4 = alt.Chart(df_view.dropna(subset=["comp_ratio_runs"])).transform_density('comp_ratio_runs', as_=['comp_ratio_runs','density']).mark_area(opacity=0.5).encode(
                x=alt.X('comp_ratio_runs:Q', title='Completion to runs'), y='density:Q'
            ).properties(width=chart_w, height=int(chart_h*0.6), title="Distribution — Pass completion to runs")
            st.altair_chart(chart4, use_container_width=True)
            st.download_button("Download HTML — Completion distribution", chart4.to_html(), file_name="runs_completion_distribution.html")
...
with TAB_PRESS:
    ...
    if press_df_raw.empty:
        ...
        for canon, aliases in PRESS_EXPECTED.items():
            ...
            for alias in aliases + [canon]:
                ...
                if alias_norm in norm_cols_p:
                    found = norm_cols_p[alias_norm]
                    break
                if alias_norm in norm_cols_p: found = norm_cols_p[alias_norm]; break
                for k_norm, orig in norm_cols_p.items():
                    if alias_norm in k_norm:
                        found = orig
                        break
        ...
        cols2 = st.columns(3)
        items2 = list(PRESS_EXPECTED.keys())
        cols2 = st.columns(3); items2 = list(PRESS_EXPECTED.keys())
        for i, canon in enumerate(items2):
            with cols2[i % 3]:
                options = [None] + list(press_df_raw.columns)
                mapping_p[canon] = st.selectbox(
                ...
                mapping_p[canon] = st.selectbox(f"{canon}", options=options, index=(options.index(auto_map_p[canon]) if auto_map_p[canon] in options else 0), key=f"map_press_{canon}")
        required_p = ["player","pressures_pm","pass_att_press_pm","comp_ratio_press","retentions_press_pm","forced_losses_press_pm"]
        missing_p = [c for c in required_p if mapping_p.get(c) is None]
        if missing_p:
@@ -336,299 +215,202 @@ def _apply_color(enc):
            cols_to_take_p = [c for c in mapping_p.values() if c is not None]
            dfp = press_df_raw[cols_to_take_p].copy()
            dfp.columns = [k for k, v in mapping_p.items() if v is not None]
            ...
            for opt in ["team","third","channel","minutes_pm","comp_ratio_press_danger","comp_ratio_press_difficult"]:
                ...
                if opt not in dfp.columns: dfp[opt] = np.nan
            eps = 1e-9
            dfp['press_load_pm'] = dfp['pressures_pm']
            dfp['pass_share_under_pressure'] = dfp['pass_att_press_pm'] / (dfp['press_load_pm'] + eps)
            dfp['retention_rate_under_pressure'] = dfp['retentions_press_pm'] / (dfp['press_load_pm'] + eps)
            ...
            bench_cols = ['OPR','OPR_danger','OPR_difficult','risk_index','comp_ratio_press','comp_ratio_press_danger','comp_ratio_press_difficult','pass_share_under_pressure','safe_action_rate']
            for col in bench_cols:
            for col in ['OPR','OPR_danger','OPR_difficult','risk_index','comp_ratio_press','comp_ratio_press_danger','comp_ratio_press_difficult','pass_share_under_pressure','safe_action_rate']:
                dfp[f'p_{col}'] = pct(dfp[col])
            ...
            if sel_channel_p:
                maskp &= dfp["channel"].isin(sel_channel_p)
            if not pd.isna(dfp["minutes_pm"]).all(): maskp &= (dfp["minutes_pm"].fillna(0) >= min_minutes_p)
            if sel_teams_p: maskp &= dfp["team"].isin(sel_teams_p)
            if sel_third_p: maskp &= dfp["third"].isin(sel_third_p)
            if sel_channel_p: maskp &= dfp["channel"].isin(sel_channel_p)
            dfp_view = dfp.loc[maskp].copy()
            ...
            cols_press = ['player','team','third','channel','minutes_pm','pressures_pm','pass_att_press_pm','pass_share_under_pressure','retentions_press_pm','forced_losses_press_pm','retention_rate_under_pressure','forced_loss_rate_under_pressure','comp_ratio_press','comp_ratio_press_danger','comp_ratio_press_difficult','OPR','OPR_danger','OPR_difficult','risk_index','throughput_succ_pass_pm','safe_action_rate']
            leader_press = dfp_view.sort_values(['OPR','risk_index','safe_action_rate'], ascending=[False, False, False])[cols_press].head(n_top_p)
            st.dataframe(leader_press.round(3), use_container_width=True)
            st.download_button("Download CSV — Under-pressure metrics", leader_press.to_csv(index=False).encode("utf-8"), file_name="under_pressure_metrics.csv")
            ...
            c1,c2,c3 = st.columns([1,1,1])
            with c1: chart_w = st.number_input("Chart width (px)", 800, 4000, 1600, step=100, key="press_w")
            with c2: chart_h = st.number_input("Chart height (px)", 400, 2400, 900, step=50, key="press_h")
            with c3: point_size = st.number_input("Point size", 20, 300, 90, step=5, key="press_pts")
            color_choice_p = st.selectbox("Color points by", ["team","third","channel","None"], index=0, key="press_color_choice")
            ...
            def _apply_color_p(enc):
                if color_choice_p != "None":
                    if color_choice_p == "team":
                        return enc.encode(color=alt.Color("team:N", scale=TEAM_COLOR_SCHEME, legend=alt.Legend(title="Team")))
                    if color_choice_p == "team": return enc.encode(color=alt.Color("team:N", scale=TEAM_COLOR_SCHEME, legend=alt.Legend(title="Team")))
                    return enc.encode(color=alt.Color(f"{color_choice_p}:N"))
                return enc
            ...
            def _save_html(chart, name): return st.download_button(name, chart.to_html(), file_name=name.replace(" ", "_").lower())
            choice = st.selectbox("Choose a visualization", ["OPR vs Forced-loss rate","Pass-share under pressure vs Completion","OPR (danger) vs OPR (difficult)","Bar: Top OPR","Histogram: Forced-loss rate","Heatmap: Decision vs Completion"], key="press_vis")
            if choice == "OPR vs Forced-loss rate":
                c = alt.Chart(dfp_view.dropna(subset=['OPR','forced_loss_rate_under_pressure']))\
                    .mark_circle(size=point_size).encode(
                        x=alt.X('forced_loss_rate_under_pressure:Q', title='Forced-loss rate (↓ better)'),
                ...
                c = alt.Chart(dfp_view.dropna(subset=['OPR','forced_loss_rate_under_pressure'])).mark_circle(size=point_size).encode(
                    x=alt.X('forced_loss_rate_under_pressure:Q', title='Forced-loss rate (↓ better)'),
                    y=alt.Y('OPR:Q', title='Overcome Pressure Rate (↑ better)'),
                    tooltip=['player','team','OPR','forced_loss_rate_under_pressure','pressures_pm','pass_att_press_pm']
                ); c = _apply_color_p(c).properties(width=chart_w, height=chart_h, title="Pressure resilience"); st.altair_chart(c, use_container_width=True); _save_html(c, "press_opr_vs_forcedloss.html")
            elif choice == "Pass-share under pressure vs Completion":
                c = alt.Chart(dfp_view.dropna(subset=['pass_share_under_pressure','comp_ratio_press']))\
                    .mark_circle(size=point_size).encode(
                        x=alt.X('pass_share_under_pressure:Q', title='Pass share under pressure'),
                ...
                c = alt.Chart(dfp_view.dropna(subset=['pass_share_under_pressure','comp_ratio_press'])).mark_circle(size=point_size).encode(
                    x=alt.X('pass_share_under_pressure:Q', title='Pass share under pressure'),
                    y=alt.Y('comp_ratio_press:Q', title='Pass completion under pressure'),
                    tooltip=['player','team','pass_share_under_pressure','comp_ratio_press','OPR']
                ); c = _apply_color_p(c).properties(width=chart_w, height=chart_h, title="Style vs execution under pressure"); st.altair_chart(c, use_container_width=True); _save_html(c, "press_style_vs_execution.html")
            elif choice == "OPR (danger) vs OPR (difficult)":
                c = alt.Chart(dfp_view.dropna(subset=['OPR_danger','OPR_difficult']))\
                    .mark_circle(size=point_size).encode(
                        x=alt.X('OPR_danger:Q', title='OPR (danger)'),
                ...
                c = alt.Chart(dfp_view.dropna(subset=['OPR_danger','OPR_difficult'])).mark_circle(size=point_size).encode(
                    x=alt.X('OPR_danger:Q', title='OPR (danger)'),
                    y=alt.Y('OPR_difficult:Q', title='OPR (difficult)'),
                    tooltip=['player','team','OPR_danger','OPR_difficult','pressures_pm']
                ); c = _apply_color_p(c).properties(width=chart_w, height=chart_h, title="Value vs technical difficulty (under pressure)"); st.altair_chart(c, use_container_width=True); _save_html(c, "press_opr_danger_vs_difficult.html")
            elif choice == "Bar: Top OPR":
                df_bar = dfp_view.sort_values('OPR', ascending=False).head(15)
                c = alt.Chart(df_bar).mark_bar().encode(
                    x=alt.X('OPR:Q', title='Overcome Pressure Rate'),
                ...
                ).properties(width=chart_w, height=int(chart_h*0.8), title="Top OPR"); st.altair_chart(c, use_container_width=True); _save_html(c, "press_top_opr_bar.html")
            elif choice == "Histogram: Forced-loss rate":
                c = alt.Chart(dfp_view.dropna(subset=['forced_loss_rate_under_pressure']))\
                    .mark_bar(opacity=0.85).encode(
                        x=alt.X('forced_loss_rate_under_pressure:Q', bin=alt.Bin(maxbins=25), title='Forced-loss rate'),
                ...
                c = alt.Chart(dfp_view.dropna(subset=['forced_loss_rate_under_pressure'])).mark_bar(opacity=0.85).encode(
                    x=alt.X('forced_loss_rate_under_pressure:Q', bin=alt.Bin(maxbins=25), title='Forced-loss rate'),
                    y=alt.Y('count()', title='Count'),
                    color=alt.Color('team:N', scale=TEAM_COLOR_SCHEME, legend=alt.Legend(title='Team'))
                ).properties(width=chart_w, height=int(chart_h*0.8), title="Distribution — Forced-loss rate"); st.altair_chart(c, use_container_width=True); _save_html(c, "press_forcedloss_hist.html")
            elif choice == "Heatmap: Decision vs Completion":
                c = alt.Chart(dfp_view.dropna(subset=['pass_share_under_pressure','comp_ratio_press']))\
                    .mark_rect().encode(
                        x=alt.X('pass_share_under_pressure:Q', bin=alt.Bin(maxbins=14), title='Pass share'),
            ...
            # Exports
            st.markdown("### Exports")
                c = alt.Chart(dfp_view.dropna(subset=['pass_share_under_pressure','comp_ratio_press'])).mark_rect().encode(
                    x=alt.X('pass_share_under_pressure:Q', bin=alt.Bin(maxbins=14), title='Pass share'),
                    y=alt.Y('comp_ratio_press:Q', bin=alt.Bin(maxbins=14), title='Completion'),
                    color=alt.Color('count():Q', title='Count')
                ).properties(width=chart_w, height=chart_h, title="Decision vs completion (binned)"); st.altair_chart(c, use_container_width=True); _save_html(c, "press_heatmap_decision_completion.html")
            bufp = io.BytesIO()
            with pd.ExcelWriter(bufp, engine="xlsxwriter") as xw:
                leader_press.to_excel(xw, sheet_name="Under Pressure — top", index=False)
                dfp_view.to_excel(xw, sheet_name="Under Pressure — mapped", index=False)
    ...
    if runs_df_raw.empty and press_df_raw.empty:
        ...
        if not runs_df_raw.empty:
            take = [c for c in [
                ...
                'Count completed passes for Runs per Match','Count completed passes leading to shot for Runs per Match','Count completed passes leading to goal for Runs per Match',
                'Count opportunities to pass to dangerous Runs per Match','Count pass attempts for dangerous Runs per Match','Count completed passes for dangerous Runs per Match'] if c in runs_df_raw.columns]
            take = [c for c in ['Player','Team','team','player','Count pass attempts for Runs per Match','Pass completion ratio to Runs','Threat of Runs to which a pass was completed per Match','Count completed passes for Runs per Match','Count completed passes leading to shot for Runs per Match','Count completed passes leading to goal for Runs per Match','Count opportunities to pass to dangerous Runs per Match','Count pass attempts for dangerous Runs per Match','Count completed passes for dangerous Runs per Match'] if c in runs_df_raw.columns]
            if any(c in take for c in ['Player','player']):
                tcol = 'Team' if 'Team' in runs_df_raw.columns else ('team' if 'team' in runs_df_raw.columns else None)
                pcol = 'Player' if 'Player' in runs_df_raw.columns else 'player'
                g = runs_df_raw[take].copy()
                ...
                g = g.reset_index().rename(columns={pcol:'player'})
                pieces.append(g)
                if tcol and tcol not in g.columns and 'team' in g.columns: tcol = 'team'
                g['team'] = g[tcol] if (tcol in g.columns) else np.nan
                ...
                if len(num_cols) > 0:
                    ...
                    g_agg = g_agg.rename(columns={pcol:'player'}); pieces.append(g_agg)
        if not press_df_raw.empty:
            takep = [c for c in [
                'Player','Team','team','player',
                'Count Pressures received per Match','Count pass attempts under Pressure per Match','Pass completion ratio under Pressure',
                'Dangerous pass completion ratio under Pressure','Difficult pass completion ratio under Pressure','Count ball retentions under Pressure per Match','Count forced losses under Pressure per Match'] if c in press_df_raw.columns]
            takep = [c for c in ['Player','Team','team','player','Count Pressures received per Match','Count pass attempts under Pressure per Match','Pass completion ratio under Pressure','Dangerous pass completion ratio under Pressure','Difficult pass completion ratio under Pressure','Count ball retentions under Pressure per Match','Count forced losses under Pressure per Match'] if c in press_df_raw.columns]
            if any(c in takep for c in ['Player','player']):
                tcol = 'Team' if 'Team' in press_df_raw.columns else ('team' if 'team' in press_df_raw.columns else None)
                pcol = 'Player' if 'Player' in press_df_raw.columns else 'player'
                gp = press_df_raw[takep].copy()
                ...
                if len(num_cols_p) > 0:
                    ...
                    gp_agg = gp_agg.rename(columns={pcol:'player'}); pieces.append(gp_agg)
        if not pieces:
            st.warning("Could not form per-player aggregates from your CSVs. Ensure 'Player' column exists.")
            ...
            # Normalize selected columns
            for p in pieces[1:]: dfp = pd.merge(dfp, p, on=['player','team'], how='outer')
            def norm01(s: pd.Series) -> pd.Series:
                s = pd.to_numeric(s, errors='coerce')
                lo, hi = np.nanpercentile(s, 5), np.nanpercentile(s, 95)
                if not np.isfinite(lo) or not np.isfinite(hi) or hi == lo:
            ...
            if len(players) == 0:
                ...
                try:
                    ...
                    radar.draw_circles(ax=ax, facecolor="#f3f3f3", edgecolor="#c9c9c9", alpha=0.18)
                p1 = st.selectbox('Player A', players, key="radar_player_a")
                p2 = st.selectbox('Player B (optional)', ['—'] + players, key="radar_player_b"); p2 = None if p2 == '—' else p2
                ...
                metrics_sel = st.multiselect('Radar metrics (3–12)', options=metric_options, default=default_metrics, key="radar_metrics_sel")
                if len(metrics_sel) < 3: st.info("Select at least 3 metrics.")
                else:
                    try:
                        radar.spoke(ax=ax, color="#c9c9c9", linestyle="--", alpha=0.18)
                    except Exception:
                    ...
                    with colp1:
                        ...
                        radar.draw_radar(v_a, ax=ax, kwargs_radar={"facecolor":"#2A9D8F33","edgecolor":"#2A9D8F","linewidth":2})
                        if v_b is not None: radar.draw_radar(v_b, ax=ax, kwargs_radar={"facecolor":"#E76F5133","edgecolor":"#E76F51","linewidth":2})
                        radar.draw_range_labels(ax=ax, fontsize=10); radar.draw_param_labels(ax=ax, fontsize=11)
                        subtitle = team_sel if team_sel != '— All —' else 'All teams'
                        ax.set_title((p1 if p2 is None else f"{p1} vs {p2}") + f" — {subtitle}", fontsize=16, pad=18)
                        st.pyplot(fig, use_container_width=True)
                        buf_png = io.BytesIO(); fig.savefig(buf_png, format='png', dpi=300, bbox_inches='tight'); buf_png.seek(0)
...
with TAB_NOTES:
    ...
    st.markdown(
        ...
        **References (non-exhaustive)**
        - Merlin et al. (2022) — determinants of pass difficulty (receiver, trajectory, zones, passer).# Streamlit app — SkillCorner Analyzer (Runs + Under Pressure + Player Radar + About)
# Author: GPT-5 Thinking
# How to run:
#   1) pip install -r requirements.txt
#   2) streamlit run app_skillcorner_integrated.py
